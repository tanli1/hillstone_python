...
def step3(dict2):
    ...
    for key, value in dict2.items():
        if isinstance(dict2[key], str):
            ...
            if isinstance(dict2[key][0], dict):
                for key1, value1 in dict2[key][0].items():     # 这里不能写key, value不然和上面的key,value混淆了
                    if isinstance(dict2[key][0][key1], str):
                        dict2[key][0][key1] = 'test'
                    elif isinstance(dict2[key][0][key1], int):
                        dict2[key][0][key1] = 100
